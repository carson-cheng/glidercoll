...
static always_inline void ObjCellList_make_empty (ObjCellList *obj, ObjCellList_Cell *cell_array, i32 max_cells)
{
	if (!obj)
...
static never_inline void ObjCellList_make_zero_size (ObjCellList *obj)
{
	if (!obj)
		return (void) ffsc (__func__);
...
static always_inline void ObjCellList_clear (ObjCellList *obj)
{
...
static never_inline int ObjCellList_add_on_cell (ObjCellList *obj, i32 x, i32 y)
{
	if (!obj || (!obj->cell && obj->max_cells != 0))
	...
	i32 rel_x = x - new_rect.left_x;
	i32 rel_y = y - new_rect.top_y;
	
	i32 insertion_ix = obj->cell_cnt;
...
static never_inline int ObjCellList_parse_rle (const char *spec, ObjCellList *obj)
{
	...
	while (TRUE)
	{
		...
		if (c >= '0' && c <= '9')
		{
			...
			u64 number;
			int success = parse_u64 (&spec, &number, NULL);
	...
	else
	{
		...
		for (cell_ix = 0; cell_ix < obj->cell_cnt; cell_ix++)
		{
			obj->cell [cell_ix].x = (u8) (((i32) obj->cell [cell_ix].x) - pop_x_on);
			obj->cell [cell_ix].y = (u8) (((i32) obj->cell [cell_ix].y) - pop_y_on);
		}
...
static always_inline int ObjCellList_copy (const ObjCellList *src_obj, ObjCellList *dst_obj)
{
...
static always_inline void ObjCellList_set_top_left (ObjCellList *obj, i32 left_x, i32 top_y)
{
	if (!obj || (!obj->cell && obj->max_cells != 0))
		return (void) ffsc (__func__);
...
static always_inline int ObjCellList_sort_are_sorted (ObjCellList_Cell heap [], i32 a_ix, i32 b_ix)
{
...
static always_inline void ObjCellList_sort_swap (ObjCellList_Cell heap [], i32 a_ix, i32 b_ix)
{
	i32 temp_x = heap [a_ix].x;
	i32 temp_y = heap [a_ix].y;
...
static always_inline void ObjCellList_sort_sift_down (ObjCellList_Cell heap [], i32 heap_size, i32 parent_ix)
{
	i32 ix_of_highest = parent_ix;
	while (TRUE)
	{
		i32 left_child_ix = (2 * parent_ix) + 1;
		i32 right_child_ix = (2 * parent_ix) + 2;
...
static always_inline void ObjCellList_sort_heapify (ObjCellList_Cell cell [], i32 cell_cnt)
{
	i32 parent_ix = (cell_cnt / 2) - 1;
	while (parent_ix >= 0)
	{
...
static never_inline void ObjCellList_sort (ObjCellList *obj)
{
	if (!obj || (!obj->cell && obj->max_cells != 0))
		return (void) ffsc (__func__);
...
static never_inline void ObjCellList_flip_horizontally (ObjCellList *obj)
{
	if (!obj || (!obj->cell && obj->max_cells != 0))
		return (void) ffsc (__func__);
	...
	i32 cell_ix;
	for (cell_ix = 0; cell_ix < obj->cell_cnt; cell_ix++)
		obj->cell [cell_ix].x = (u8) ((obj->obj_rect.width - 1) - (i32) obj->cell [cell_ix].x);
	
	ObjCellList_sort (obj);
}

static never_inline void ObjCellList_flip_vertically (ObjCellList *obj)
{
	if (!obj || (!obj->cell && obj->max_cells != 0))
		return (void) ffsc (__func__);
	...
	i32 cell_ix;
	for (cell_ix = 0; cell_ix < obj->cell_cnt; cell_ix++)
		obj->cell [cell_ix].y = (u8) ((obj->obj_rect.height - 1) - (i32) obj->cell [cell_ix].y);
	
	ObjCellList_sort (obj);
}

static never_inline void ObjCellList_flip_diagonally (ObjCellList *obj)
{
	if (!obj || (!obj->cell && obj->max_cells != 0))
		return (void) ffsc (__func__);
	...
	i32 prev_width = obj->obj_rect.width;
...
static never_inline int ObjCellList_evolve_slow (const ObjCellList *in_obj, ObjCellList *out_obj)
{
	...
	i32 out_cell_ix = 0;
	i32 out_pop_x_on = in_obj->obj_rect.height + 1;
	i32 out_pop_x_off = -1;
	...
	for (mid_y = -1; mid_y < in_obj->obj_rect.height + 1; mid_y++)
		for (mid_x = -1; mid_x < in_obj->obj_rect.width + 1; mid_x++)
		{
			...
			{
				i32 cell_x = (i32) in_obj->cell [in_cell_ix].x;
				i32 cell_y = (i32) in_obj->cell [in_cell_ix].y;
	...
	i32 out_pop_y_on = ((i32) out_obj->cell [0].y) - 1;
	i32 out_pop_y_off = (((i32) out_obj->cell [out_obj->cell_cnt - 1].y) + 1) - 1;
	
	for (out_cell_ix = 0; out_cell_ix < out_obj->cell_cnt; out_cell_ix++)
	{
		out_obj->cell [out_cell_ix].x = (u8) (((i32) out_obj->cell [out_cell_ix].x) - (out_pop_x_on + 1));
		out_obj->cell [out_cell_ix].y = (u8) (((i32) out_obj->cell [out_cell_ix].y) - (out_pop_y_on + 1));
	}
