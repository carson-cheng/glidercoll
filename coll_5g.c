...
static never_inline int do_search (const char *spec)
{
	...
	while (TRUE)
	{
		...
		// if (seeds_cnt % 16777216 == 0)
		{
            ...
            if (time(NULL) - start > 41400){
                // 11 hours and 30 seconds have elapsed, the session is about the terminate
                return EXIT_SUCCESS;
            }
		}
		...
		if (will_interfere (&gl_1, &gl_2) || will_interfere (&gl_1, &gl_3) || will_interfere (&gl_1, &gl_4) || will_interfere (&gl_1, &gl_5) || will_interfere (&gl_2, &gl_3) || will_interfere (&gl_2, &gl_4) || will_interfere (&gl_2, &gl_5) || will_interfere (&gl_3, &gl_4) || will_interfere (&gl_3, &gl_5) || will_interfere (&gl_4, &gl_5))
		{
			interferes_cnt++;
			continue;
		}
...
static never_inline int main_do (int argc, const char *const *argv)
{
	...
	char *spec = (char*) malloc (SPEC_BUF_SIZE);
